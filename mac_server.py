...
from tools.system import (
    ...
    get_date_time as get_date_time_impl
)
...
from tools.app_integrations import (
    mail_tool as mail_tool_impl,
    calendar_tool as calendar_tool_impl,
    messages_tool as messages_tool_impl,
    safari_tool as safari_tool_impl,
    notes_tool as notes_tool_impl,
    terminal_tool as terminal_tool_impl,
...
# ==================== APP INTEGRATIONS ====================
@mcp.tool()
def mail(action: str, account: Optional[str] = None, limit: int = 10, message_id: Optional[str] = None, to: Optional[str] = None, subject: Optional[str] = None, body: Optional[str] = None, query: Optional[str] = None) -> str:
    """Interact with macOS Mail app: 'read_inbox', 'read_message', 'send_draft', or 'search'."""
    return mail_tool_impl(action, account, limit, message_id, to, subject, body, query)

@mcp.tool()
def calendar(action: str, calendar_name: Optional[str] = None, limit: int = 10, title: Optional[str] = None, start_time: Optional[str] = None, end_time: Optional[str] = None, location: Optional[str] = None, event_id: Optional[str] = None) -> str:
    """Interact with macOS Calendar app: 'list_events', 'create_event', or 'delete_event'."""
    return calendar_tool_impl(action, calendar_name, limit, title, start_time, end_time, location, event_id)

@mcp.tool()
def messages(action: str, recipient: Optional[str] = None, text: Optional[str] = None, limit: int = 10) -> str:
    """Interact with iMessage/Messages app: 'send' or 'read_recent'."""
    return messages_tool_impl(action, recipient, text, limit)

@mcp.tool()
def safari(action: str, url: Optional[str] = None, js_script: Optional[str] = None) -> str:
    """Interact with Safari app: 'open_url', 'get_active_tab', 'list_open_tabs', or 'run_js_on_active_tab'."""
    return safari_tool_impl(action, url, js_script)

@mcp.tool()
def notes(action: str, title: Optional[str] = None, body: Optional[str] = None, query: Optional[str] = None, note_id: Optional[str] = None) -> str:
    """Interact with Notes.app: 'create', 'search', or 'append'."""
    return notes_tool_impl(action, title, body, query, note_id)

@mcp.tool()
def terminal(action: str, command: Optional[str] = None, text: Optional[str] = None, target: str = "terminal") -> str:
    ...
    return terminal_tool_impl(action, command, text, target)
